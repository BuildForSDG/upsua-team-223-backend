@extends('layouts.app', ['title' => __('Finance Costs Management')])

@section('content')
    @include('users.partials.header', ['title' => __('Edit Finance Costs')])
                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ __('Finance costs management') }}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ route('finance.show',$cost->finance->id) }}" class="btn btn-sm btn-primary">{{ __('Returns to the list') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{ route('financecost.update', $cost) }}" autocomplete="off">
                            @csrf
                            @method('put')

                            <h6 class="heading-small text-muted mb-4">{{ __('Finance cost information') }}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('min') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-min">{{ __('Min value') }}</label>
                                    <input type="hidden" value="{{ $cost->finance->id }}" name="finance_id">
