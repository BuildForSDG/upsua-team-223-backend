...
@section('content')
    @include('users.partials.header', ['title' => __('Edit Locality')])
        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('locality.update', $locality) }}" autocomplete="off">
                            @csrf
                            @method('put')
                           ...
                           <h6 class="heading-small text-muted mb-4">{{ __('Locality information') }}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" value="{{ $locality->name }}" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name ...') }}" value="{{ old('name') }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('subdivision') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="subdivision" value="{{ $locality->subdivision }}" id="input-subdivision" class="form-control form-control-alternative{{ $errors->has('subdivision') ? ' is-invalid' : '' }}" placeholder="{{ __('Subdivision ...') }}" value="{{ old('subdivision') }}" required autofocus>
